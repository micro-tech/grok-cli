...
-  **`.zed/`** - Zed editor settings, tasks.json for task tracking
...
-   Use short descrpited names for varabils names that tell what it dosw and doing.
...
- Store documentation notes in `.Doc/docs` directory
...
- Use `.config.toml` for project-specific   and `.grok/config.toml` Grok CLI settings     
...
- **Tasks**: See `.zed/tasks_list.json` for project task list
