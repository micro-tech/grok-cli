...
pub fn get_all_settings(config: &Config) -> Vec<SettingDefinition> {
    ...
    settings.extend(vec![
        ...
        SettingDefinition {
            key: "acp.require_permission".to_string(),
            label: "Require Tool Permission".to_string(),
            description: "Show Allow / Always Allow / Cancel dialog before each tool call. Set false to auto-approve all tools (YOLO mode).".to_string(),
            category: "ACP".to_string(),
            setting_type: SettingType::Boolean,
            default_value: "true".to_string(),
            requires_restart: false,
            current_value: config.acp.require_permission.to_string(),
        },
        SettingDefinition {
            key: "acp.permission_timeout_secs".to_string(),
            label: "Permission Dialog Timeout (secs)".to_string(),
            description: "How long to wait for you to click Allow/Deny before the tool call times out. Default 300 s — increase if you need more reading time.".to_string(),
            category: "ACP".to_string(),
            setting_type: SettingType::Number,
            default_value: "300".to_string(),
            requires_restart: false,
            current_value: config.acp.permission_timeout_secs.to_string(),
        },
        SettingDefinition {
            key: "acp.max_tool_loop_iterations".to_string(),
            label: "Max Tool Loop Iterations".to_string(),
            description: "Maximum number of back-and-forth tool calls allowed per prompt before Grok gives up. Increase for complex multi-step tasks.".to_string(),
            category: "ACP".to_string(),
            setting_type: SettingType::Number,
            default_value: "25".to_string(),
            requires_restart: false,
            current_value: config.acp.max_tool_loop_iterations.to_string(),
        },
        SettingDefinition {
            key: "acp.max_history_messages".to_string(),
            label: "Max History Messages".to_string(),
            description: "Maximum conversation turns kept in session context. Older messages are trimmed to prevent unbounded token growth.".to_string(),
            category: "ACP".to_string(),
            setting_type: SettingType::Number,
            default_value: "40".to_string(),
            requires_restart: false,
            current_value: config.acp.max_history_messages.to_string(),
        },
    ]);
