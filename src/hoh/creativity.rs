...
impl CreativityEngine {
    ...
    /// 453 close-the-loop: Convert high-quality creative ideas into safe TaskMutation::AddTask.
    /// These can be fed into TaskEvolutionEngine / TaskMutationRules to create real tasks.
    pub fn ideas_to_add_task_mutations(
        &self,
        ...
        ideas
            ...
            .enumerate()
            .map(|(idx, idea)| {
                let new_id = base_id + (idx as u64) + 1;
                TaskMutation::AddTask {
                    new_task: Task {
                        id: new_id,
                        title: format!("[HOH-401] {}", idea.title),
                        description: idea.description.clone(),
                        details: format!(
                            "Autonomously generated by HOH Creativity Engine.\n\n\
                             Overall score: {:.2} (novelty={:.2}, feasibility={:.2}, alignment={:.2})\n\
                             Source: {}\n\
                             Provenance: {}\n\n\
                             Implementation hint: {}\n\n\
                             This task was materialized from a creative idea (453).",
                            idea.overall_score,
                            idea.novelty_score,
                        ...
                        test_strategy: format!(
                            "1. Implement the core idea described above.\n\
                             2. Add unit tests in the appropriate module.\n\
                             3. Wire any new behavior into HOHPlanner or outer_loop if applicable.\n\
                             4. Verify the idea produces measurable improvement (use simulation mode first)."
                        ),
                        ..Default::default()
                    },
                }
