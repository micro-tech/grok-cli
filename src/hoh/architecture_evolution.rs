...
pub struct ArchitectureEvolutionEngine {
    ...
    /// Project root so diagnostic / stub artifacts are written under .grok/hoh/scratch/
    /// instead of polluting src/ or .zed/.
...
impl ArchitectureEvolutionEngine {
    ...
    /// Preferred constructor when the real project root is known.
    ...
    //
    // IMPORTANT HYGIENE RULE: These stubs are *diagnostic / proposal* artifacts only.
    // Real file writes for 361.x materialization must go through scratch_dir().
    // The files_changed here describe the *intended future target* for human review,
    // but any generated PatchSet content we emit as a stub is written to .grok/hoh/scratch/.
    ...
    fn create_patch_stub_for_split(&self, module: &str) -> PatchSet {
        ...
        let stub_name = format!("arch_split_{}.rs", module.replace(|c: char| !c.is_alphanumeric(), "_"));

        PatchSet {
            id: format!("arch-split-{}", uuid::Uuid::new_v4()),
            // We still record the *intended* real location for traceability,
            // but the actual diagnostic stub will be materialized under scratch.
            files_changed: vec![
                format!("src/hoh/{}.rs", module),
                format!("src/hoh/{}_mod.rs", module),
                scratch.join(&stub_name).to_string_lossy().to_string(),
            ],
            diff_summary: format!(
                "ARCH-SPLIT: Extract domain concerns from {} into focused submodules.\n\
                 - Intended: src/hoh/{}/mod.rs (real work)\n\
                 - Diagnostic stub written to: {}\n\
                ...
                module, module, scratch.join(&stub_name).display()
            ),
    ...
    fn create_layer_extraction_patch(&self, layer: &str) -> PatchSet {
        ...
        let scratch = crate::hoh::persistence::scratch_dir(&self.project_root);
        let safe = layer.replace(|c: char| !c.is_alphanumeric() && c != '_', "_");
        let stub_name = format!("arch_layer_{}.rs", safe);

        PatchSet {
            id: format!("arch-layer-{}", uuid::Uuid::new_v4()),
            files_changed: vec![
                format!("src/hoh/{}.rs", layer.replace('/', "_")),
                scratch.join(&stub_name).to_string_lossy().to_string(),
            ],
            diff_summary: format!(
                "ARCH-LAYER: Introduce dedicated {} layer.\n\
                 - Intended location: src/hoh/{}.rs\n\
                 - Diagnostic stub: {}\n\
                 - New module with clear public API\n\
                 - Move architecture reasoning logic here\n\
                 (simulation-safe stub for 361.1)",
                layer, layer.replace('/', "_"), scratch.join(&stub_name).display()
            ),
            source: "architecture_evolution".to_string(),
            timestamp: now,
            intended_content: None,
    ...
    fn create_trait_introduction_patch(&self, target: &str) -> PatchSet {
        ...
        let scratch = crate::hoh::persistence::scratch_dir(&self.project_root);
        let safe = target.replace(|c: char| !c.is_alphanumeric() && c != '_', "_");
        let stub_name = format!("arch_trait_{}.rs", safe);

        PatchSet {
            id: format!("arch-trait-{}", uuid::Uuid::new_v4()),
            files_changed: vec![
                format!("src/hoh/{}.rs", target),
                scratch.join(&stub_name).to_string_lossy().to_string(),
            ],
            diff_summary: format!(
                ...
                target, target, scratch.join(&stub_name).display(), target
            ),
            source: "architecture_evolution".to_string(),
            timestamp: now,
            intended_content: None,
...
mod tests {
    ...
    async fn test_propose_evolutions_in_simulation() {
        ...
        let engine = ArchitectureEvolutionEngine::new_with_project_root(adapter, true, dir.path().to_path_buf());
    ...
    /// Guard test for HOH hygiene (361.x).
    /// Ensures that diagnostic / proposal PatchSet stubs generated by ArchitectureEvolutionEngine
    /// never target src/ or .zed/ for actual artifact writes. All real sidecar content must go to .grok/hoh/scratch/.
    #[tokio::test]
    async fn architecture_evolution_stubs_respect_hoh_hygiene() {
        use tempfile::tempdir;
        let dir = tempdir().unwrap();
        let adapter = TaskListAdapter::new(dir.path(), true);

        // Seed minimal data to trigger proposals that produce patch stubs
        let mut list = crate::hoh::tasklist_adapter::TaskList::default();
        list.tasks.push(crate::hoh::tasklist_adapter::Task {
            id: 42,
            title: "Architecture work".to_string(),
            details: "planner evolution_engine self-refinement".to_string(),
            priority: "high".to_string(),
            status: "pending".to_string(),
        ...
        let engine = ArchitectureEvolutionEngine::new_with_project_root(adapter, true, dir.path().to_path_buf());
        let proposals = engine.propose_evolutions().await.unwrap_or_default();

        for proposal in &proposals {
            for patch in &proposal.proposed_patches {
                for file in &patch.files_changed {
                    let f = file.to_lowercase();
                    // Diagnostic stubs must not be written to src/ or .zed/
                    if f.contains("src/") || f.contains(".zed/") {
                        // It's acceptable to *mention* the intended real target in the diff_summary,
                        // but files_changed for the actual stub artifact must be under scratch.
                        // We allow the "intended" src paths only if a scratch path is also present for this stub.
                        let has_scratch = patch.files_changed.iter().any(|p| p.to_lowercase().contains("scratch") || p.to_lowercase().contains(".grok/hoh"));
                        assert!(has_scratch, "HOH hygiene violation: patch files_changed contains src/.zed path without corresponding scratch entry: {}", file);
                    }
                    // Stronger: any path that looks like a real write target for a stub must be under .grok/hoh
                    if !f.contains("scratch") && !f.contains(".grok/hoh") && (f.contains("src/") || f.contains(".zed/")) {
                        panic!("HOH hygiene regression: ArchitectureEvolution stub targets forbidden location: {}", file);
                    }
                }
            }
        }
