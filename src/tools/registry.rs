...
/// Return available tool definitions (same as get_tool_definitions for now).
pub fn get_available_tool_definitions() -> Vec<Value> {
    get_tool_definitions()
        ...
        // ── Agent coordination ───────────────────────────────────────────────
        json!({"type":"function","function":{"name":"spawn_agent","description":"Spawn a focused sub-agent to complete a well-scoped task. Requires GROK_API_KEY or XAI_API_KEY in environment.","parameters":{"type":"object","properties":{"task":{"type":"string","description":"Task description for the sub-agent"},"context":{"type":"string","description":"Relevant background context (optional)"},"max_tokens":{"type":"integer","description":"Maximum response tokens, 256–4096 (default 2048)"}},"required":["task"]}}}),
        json!({"type":"function","function":{"name":"send_message","description":"Send a message to a named target agent or channel (appended to ~/.grok/messages/{target}.jsonl)","parameters":{"type":"object","properties":{"target":{"type":"string","description":"Target agent ID or channel name"},"message":{"type":"string","description":"Message content"}},"required":["target","message"]}}}),
        json!({"type":"function","function":{"name":"team_create","description":"Create a named team configuration in ~/.grok/teams.json","parameters":{"type":"object","properties":{"name":{"type":"string"},"members":{"type":"array","items":{"type":"string"},"description":"Agent IDs or names"},"description":{"type":"string"}},"required":["name"]}}}),
        json!({"type":"function","function":{"name":"team_delete","description":"Delete a named team from ~/.grok/teams.json","parameters":{"type":"object","properties":{"name":{"type":"string"}},"required":["name"]}}}),
        // ── MCP ──────────────────────────────────────────────────────────────
        json!({"type":"function","function":{"name":"mcp_call","description":"Invoke a tool on an MCP (Model Context Protocol) server. The server is launched as a subprocess via stdio.","parameters":{"type":"object","properties":{"server_command":{"type":"string","description":"Command to launch the MCP server (e.g. 'npx @modelcontextprotocol/server-github')"},"tool_name":{"type":"string","description":"Name of the MCP tool to call"},"arguments":{"type":"object","description":"Tool arguments as a JSON object"}},"required":["server_command","tool_name"]}}}),
        // ── LSP ──────────────────────────────────────────────────────────────
        json!({"type":"function","function":{"name":"lsp_query","description":"Query code intelligence for a file position. Supports diagnostics (cargo check for Rust), hover context, definition search, and reference search.","parameters":{"type":"object","properties":{"file":{"type":"string","description":"Path to the source file"},"line":{"type":"integer","description":"0-based line number"},"character":{"type":"integer","description":"0-based character offset"},"query_type":{"type":"string","enum":["diagnostics","hover","definition","references"],"description":"Type of query"}},"required":["file","query_type"]}}}),
        // ── Discovery ────────────────────────────────────────────────────────
