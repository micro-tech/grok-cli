use anyhow::Result;
use grok_cli::acp::protocol::{PermissionOutcome, RequestPermissionParams, SessionId, ToolKind};
...
async fn test_permission_proceed_once() -> Result<()> {
    ...
    unsafe { std::env::set_var("GROK_API_BASE_URL", server.url()); }
    ...
    let _m2 = server.mock("POST", "/v1/chat/completions")
        .with_status(200)
        .with_body(mock_resp(Some("Here is the directory listing."), None, "stop"))
        .expect(1)
        .create_async().await;
    ...
    tokio::spawn(async move {
        if let Some((params, tx)) = rx.recv().await {
    ...
    let response = agent.handle_chat_completion(&session_id, "list files", None, None, Some(bridge_arc)).await?;
    
    assert_eq!(response, "Here is the directory listing.");
    assert!(!agent.is_always_allowed(&session_id, "run_shell_command").await);
...
async fn test_permission_cancel() -> Result<()> {
    ...
    let _m2 = server.mock("POST", "/v1/chat/completions")
        .with_status(200)
        .with_body(mock_resp(Some("I understand you rejected the command. I won't run it."), None, "stop"))
        .expect(1)
        .create_async().await;
    ...
    let response = agent.handle_chat_completion(&session_id, "delete everything", None, None, Some(bridge_arc)).await?;
...
async fn test_permission_always_allow_persists() -> Result<()> {
    ...
    agent.handle_chat_completion(&session_id, "list files", None, None, Some(bridge_arc)).await?;
    assert!(agent.is_always_allowed(&session_id, "list_directory").await);

    agent.handle_chat_completion(&session_id, "list files again", None, None, Some(bridge_clone)).await?;
...
async fn test_permission_timeout() -> Result<()> {
    ...
    let result = agent.handle_chat_completion(&session_id, "list files", None, None, Some(bridge_arc)).await;
...
async fn test_permission_gate_disabled() -> Result<()> {
    ...
    let response = agent.handle_chat_completion(&session_id, "list files", None, None, None).await?;
